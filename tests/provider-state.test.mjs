...
import { providerCircuitStatus, readProviderState, recordProviderOutcome, recoverProviders } from "../src/lib/provider-state.mjs";
...
test("opens, classifies, and recovers a provider circuit", async () => {
  ...
  const config = { providerHealth: { failureThreshold: 3, baseOpenMs: 10, maxOpenMs: 100, latencyWarningMs: 50, windowSize: 20 } };
  ...
  circuit = providerCircuitStatus(state, "zai", config);
  assert.equal(circuit.circuit, "half-open");

  const recovery = await recoverProviders({ config, providers: ["zai"], deps: { statePath: path, probe: async () => ({ status: "ok" }) } });
